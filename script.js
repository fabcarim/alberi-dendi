...
function loadDailyLevel(size) {
    ...
    archiveDateEl.style.display = 'inline-block';
    const archiveLabel = document.querySelector('label[for="archive-date"]');
...
function startTimer(resume = false) {
    ...
    // Se stiamo riprendendo una partita già iniziata, facciamo ripartire il timer subito
    if (resume && elapsedTime > 0) {
        resumeTimer();
    }
}
...
function loadTrainingLevel() {
    ...
    archiveDateEl.style.display = 'none';
    const archiveLabel = document.querySelector('label[for="archive-date"]');
