...
// UI Elements for Archive Modal
const btnOpenArchive = document.getElementById('btn-open-archive');
const archiveModal = document.getElementById('archive-modal');
const archiveDateModalEl = document.getElementById('archive-date-modal');
...
function init() {
    ...
    archiveDateModalEl.max = realtimeTodayStr;
    const minDate = new Date(now.setMonth(now.getMonth() - 6));
    archiveDateModalEl.min = minDate.toISOString().split('T')[0];
    archiveDateModalEl.value = realtimeTodayStr;
    ...
    btnOpenArchive.addEventListener('click', () => {
        archiveDateModalEl.value = todayStr; // Pre-fill with currently loaded date
        archiveModal.classList.add('show');
    });
    btnCloseArchive.addEventListener('click', () => archiveModal.classList.remove('show'));
    archiveModal.addEventListener('click', (e) => {
        if (e.target === archiveModal) archiveModal.classList.remove('show');
    });
    
    btnLoadArchive.addEventListener('click', () => {
        let selectedDate = archiveDateModalEl.value;
        if (!selectedDate) selectedDate = realtimeTodayStr;
    ...
    btnBackToday.addEventListener('click', () => {
        ...
        todayStr = resetNow.toISOString().split('T')[0];
        const nextSize = currentSize === 12 ? 10 : currentSize;
        loadDailyLevel(nextSize);
    });
...
function loadDailyLevel(size) {
    ...
    if (todayStr === realtimeTodayStr) {
        mainTitleEl.textContent = "Alberi Daily 🌲";
        mainSubtitleEl.textContent = "Un nuovo puzzle ogni giorno!";
        dateDisplayEl.textContent = dateFormatted;
        btnBackToday.style.display = 'none';
        btnOpenArchive.style.display = 'inline-block';
    } else {
        mainTitleEl.textContent = "Alberi Daily - Archivio 📅";
        mainSubtitleEl.innerHTML = `Stai giocando nel passato: <strong>${dateFormatted}</strong>`;
        dateDisplayEl.textContent = "";
        btnBackToday.style.display = 'inline-block';
        btnOpenArchive.style.display = 'inline-block';
    }
    ...
    if (progressData) {
        try {
            ...
            startTimer(true); // Will start immediate if elapsedTime > 0
        } catch (e) {
            startTimer(false);
        }
    } else {
...
function resetLevel() {
    ...
    startTimer(false); // Resetta il timer a 0 e aspetta il clic
    saveProgress();
}
...
function loadTrainingLevel() {
    ...
    mainTitleEl.textContent = "Allenamento Libero 🏋️";
    mainSubtitleEl.textContent = "Sfide casuali illimitate senza salvataggio record.";
