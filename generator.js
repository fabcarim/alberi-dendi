const LevelGenerator = {
    ...
    placeTrees: function (size, rng) {
        // Backtracking to place 1 tree per row and col, no touching, with random row order
        const grid = Array(size).fill().map(() => Array(size).fill(0));
        const colsUsed = Array(size).fill(false);
        const rows = Array.from({ length: size }, (_, i) => i);
        rng.shuffle(rows);

        if (this.backtrack(grid, rows, 0, size, colsUsed, rng)) {
            return grid;
        }
        return null;
    },

    backtrack: function (grid, rows, rowIndex, size, colsUsed, rng) {
        ...
        for (let c of cols) {
            if (!colsUsed[c] && this.isValidPlacement(grid, r, c, size)) {
                ...
                if (this.backtrack(grid, rows, rowIndex + 1, size, colsUsed, rng)) return true;
